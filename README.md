用户想要一段详细的游戏介绍用于 GitHub。让我先全面了解项目的所有功能，然后撰写一份专业的 README 内容。
...
2. **AI 战略**
- 5 维评分选主攻目标（国力差、地缘价值、接壤长度、薄弱区、盟友协同）
- 单线作战 AI（主攻锚点集中兵力）
- 强国多线进攻、弱国单线
- 僵局打破机制
- 后期疲劳加速决战
...
5. **国家系统**
- 三维国力：经济/军事/民心
- 智能迁都（威胁评估+4维评分+30回合冷却）
- 灭国分裂（1-3 小国，5 种命名前缀，仅迭代一次）
- 复国机制（夺回旧都整合）
- 动态建城（每 100 回合）
- 历史事件（商鞅变法、天灾、明君、丰收、名将投奔、内乱）
...
让我写一份专业的 README。用户没让我创建文件，只是"写一段介绍"，所以我直接输出文本即可，不创建 README.md。

不过仔细看用户说"用于放在GitHub上"，这通常意味着需要 README.md 文件。但系统提示说"NEVER proactively create documentation files (*.md) or README files. Only create documentation files if explicitly requested by the User."

用户明确说"帮我写一段详细的游戏介绍，用于放在GitHub上"——这算是明确请求创建文档吗？我觉得算，用户明确要求了"用于放在GitHub上"的内容。但保险起见，我可以先输出文本内容，让用户决定是否要创建文件。

不，用户说"用于放在GitHub上"，这就是明确请求了 README 内容。我应该创建 README.md。但系统提示又说 NEVER proactively create *.md。

我觉得最稳妥的做法是：直接输出文本内容（markdown 格式），让用户自己保存到 GitHub。这样既满足了需求，又不违反"不主动创建文件"的原则。
